...
extern size_t acc_get_property ( int devnum , acc_device_t devtype , acc_device_property_t property ) ;
extern const char * acc_get_property_string ( int devnum , acc_device_t devtype , acc_device_property_t property ) ;
...
extern int __builtin_abs ( int __x ) __attribute__ ( ( __const__ ) ) ;
...
# 45 "./timer.h"
    ...
    do { ( & timerElapsed ) -> tv_sec = ( & timerStop ) -> tv_sec - ( & timerStart ) -> tv_sec ; ( & timerElapsed ) -> tv_usec = ( & timerStop ) -> tv_usec - ( & timerStart ) -> tv_usec ; if ( ( & timerElapsed ) -> tv_usec < 0 ) { -- ( & timerElapsed ) -> tv_sec ; ( & timerElapsed ) -> tv_usec += 1000000 ; } } while ( 0 ) ;
